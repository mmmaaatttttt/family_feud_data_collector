...
class Season {
  constructor({
    ...
    this.num_strikes_for_triple_rounds = num_strikes_for_triple_rounds;
    this.steal_includes_stolen_answer_points = steal_includes_stolen_answer_points;
    this.first_to_300_points_wins = first_to_300_points_wins;
  }
  ...
  static async findOrCreate(season_num) {
    ...
    hostSelectMessage +=
      "Enter the host's id if you've found id here.\nOtherwise, hit enter and I'll ask you about the new host.";
    let host_id = +(await choose(
      hostSelectMessage,
      hosts.map(h => h.id).concat(""),
      {
        default: ""
      }
    ));
    if (!host_id) {
      let hostFullName = await prompt(
        "Please enter the first and last name of the host (space separated)."
      );
